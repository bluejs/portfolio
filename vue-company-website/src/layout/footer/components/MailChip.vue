                ...
                <div id="mc_embed_signup">
                    <form @submit.prevent="sendEmail" novalidate>
                        ...
                        <span class="input-email is-rainbow" v-bind:class="{ 'success': subscribeSuccessful }">
                            <input type="email" name="email" v-validate="'required|email'" id="newsletter-form-email"
                                placeholder="输入邮件地址" class="placeholder hide-on-focus" onfocus="this.placeholder = ''"
                                onblur="this.placeholder = '输入邮件地址'" v-bind:disabled="emailSendClicked" v-model="email">
                            <span></span>
                        </span>
                        <div class="form-icon">
                            <button type="submit" name="submit" id="newsletter-submit"
                                ...
                                <span class="animated fadeIn" v-show="subscribeSuccessful"><i
                        ...
                        <span class="success-message" v-show="subscribeSuccessful"><span
                                class="icon is-small">🎉</span><span
                                v-html="$t('home:beta_container.success', { count: count + 1 })"></span></span>
                    </form>
                </div>
            </div>
        </div>
